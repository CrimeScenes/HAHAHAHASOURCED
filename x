--[[ 
    CRUCIAL NOTES FOR INITIAL SETUP
    - All keybinds must remain unique; assigning the same key to multiple functions, even if those functions are disabled, is not permitted.
    - Do not remove any keybinds, regardless of whether their corresponding features are inactive.
    - For safety reasons, BoxFOV is operational only with Target Silent.
    - As of now, Triggerbot functions solely with Target Silent.
    - NOTE: Textures could break your macro on some ripoffs and cause some lag.
]]
getgenv().Settings = {
    PerformanceTweaks = {
        FPSBoost = {
...
getgenv().CleanConsole = {
    Enabled = true,         
    Mode = "keybind",        -- check Keytrigger for Keybind.  
}
getgenv().Silent = {
    Startup = {
        Enabled = true,  -- Enables or disables the silent aim feature.
        mode = "target", -- 'normal' or 'target' (NOTE : if normal it uses silent without target mode.)
        Prediction = 0.1261,  
    ...
    AimSettings = {  
        FovSettings = {
            FovVisible = false,  
            FovRadius = 19,  -- NOTE : Keep as it is if BoxFov is enabled
...
getgenv().HitChances = {
    Active = true,
    WeaponStats = {
        Rev = 100, DB = 100, Shot = 100,
        TacShot = 100, SMG = 100, Sil = 100,
        AR = 100, Other = 100
    }   
}
getgenv().BoxFov = {
...
getgenv().CamLock = {
    Normal = {
        ...
        Smoothness = 0.013,  -- The degree of smoothness applied to the lock transition.
    ...
    ShakeSettings = {  
        X = 0,  -- Amplitude of camera shake along the X axis
        Y = 0,  -- Amplitude of camera shake along the Y axis
        Z = 0,  -- Amplitude of camera shake along the Z axis
    },
    EasingStyle = "Linear",  
...
getgenv().Visuals = {
    Enabled = true,  
    TracerThickness = 2,  -- Thickness of the tracer
    TracerTransparency = 0.5,  -- Transparency of the tracer
    TargetClosestBodyPart = true,  -- Target closest body part to mouse on target
}
getgenv().ErrorSuppression = {
    Enabled = true,  -- avoids errors in console.
}
getgenv().FakeSpikeConfig = {
    IsEnabled = true,               
    SpikeStrength = 500,            
    UseToggleMode = true,           -- When enabled, it switches the lag spike on or off.
    ResetDelay = 4,                 -- Time before the lag is reset to its normal state.
}
getgenv().Ranges = {
    FarDistance = 73,
    MidDistance = 42,
...
getgenv().Textures = {
    Enabled = false,
    TextureType = "Brick",  -- for more info "https://static.wikia.nocookie.net/roblox/images/c/c1/All_Materials_2022.png/revision/latest?cb=20221125005730".
    Hue = Color3.fromRGB(50, 50, 50), 
    --[[ 
    Available Materials: 
    Plastic, Wood, Metal, Brick, Slate, Glass, DiamondPlate, Concrete, Grass, Cobblestone, 
    Ice, Marble, WoodPlanks, Rock, BrickYellow, Neon, SmoothPlastic 
    ]]--
}
getgenv().AutoLowGFX = {
    Enabled = false, 
    Duration = 3,   
    Type = "keybind", -- Set to "auto" for automatic , "keybind" for key press
}
getgenv().Macro = {
    Settings = { IsEnabled = false },
    Configurations = {
        SpeedGlitch = { IsActive = false, Duration = 5, Frequency = 0.1, Description = "Speed glitch on key press." }
    }
}
getgenv().Features = {
    Settings = { StretchRes = true, AnimationPack = false, RainbowBars = false },
    Configurations = {
        Resolution = { StretchFactor = 0.70, Mode = "remain" }, -- 'toggle' or 'remain'.
        AnimationOptions = { Speed = 1, Looping = true, Effects = { Fade = true, Bounce = false } }
    }
}
getgenv().PanicSystem = {
    Settings = {
...
getgenv().Gun_Fov = {
    Settings = { IsEnabled = false, DefaultFov = 5, AdjustmentFactor = 1.0 },
    Configurations = {
        Weapons = {
            DoubleBarrel = { Fov = 3.8, Adjustment = { Dynamic = true, MaxAdjustment = 2.0 } },
            Revolver = { Fov = 4.8, Adjustment = { Dynamic = false } },
            Shotgun = { Fov = 5, Adjustment = { Dynamic = true, MaxAdjustment = 1.5 } },
            TacticalShotgun = { Fov = 1.9, Adjustment = { Dynamic = true, MaxAdjustment = 2.5 } },
            Smg = { Fov = 9, Adjustment = { Dynamic = true, MaxAdjustment = 1.8 } },
            Silencer = { Fov = 2, Adjustment = { Dynamic = false } },
        }
    }
}
getgenv().ModCheck = {
    Enabled = true, 
        KickIfModJoined = false,  
        NotificationDuration = 5,
}
getgenv().Rejoin = {
    Enabled = false,
    Delay = { UseDelay = true, Duration = 1 }
}
getgenv().Spin = {
    Enabled = true,  
    Motion = {
...
getgenv().PlayerPredictions = { -- // DO NOT TOUCH THE RANGE // --
    LowPing = {
        Range = {20, 30}, 
        PredictionFactor = 0.105, -- change predictionMultiplier
    },
    ModeratePing = {
        Range = {30, 50}, 
    ...
    AveragePing = {
        Range = {50, 70}, 
        PredictionFactor = 0.122, 
    },
    HigherPing = {
        Range = {70, 90}, 
        PredictionFactor = 0.128, 
    },
    HighPing = {
        Range = {90, 120}, 
